...
load('S192803_Laxity_Data_Processed_2.mat');
...
%     experiment_laxity_trial(count_trial).angle=angle;
    ...
    temp_str=strsplit(char(jobParams.jobName{count_trial}),'_');
    angle_str=strsplit(char(temp_str{5}),'deg');
    angle=str2num(angle_str{1});
    ...
    if testCharPresentInChar(lax_type,'Anterior')
        ...
        elseif angle==0
            A0=[A0,abs(kin_sim(count_trial,5)-kin_exp(count_trial,5))];
            AIE0=[AIE0,abs(kin_sim(count_trial,3)-kin_exp(count_trial,3))];
            AVV0=[AVV0, abs(kin_sim(count_trial,2)-kin_exp(count_trial,2))];
        end
...
rmse(13) = A0/(max(kin_exp(:,5))-min(kin_exp(:,5)));
rmse(14) = AIE0/(max(kin_exp(:,3))-min(kin_exp(:,3)));
rmse(15) = AVV0/(max(kin_exp(:,2))-min(kin_exp(:,2)));


error_fid=fopen('trial_error_S192803.txt','a+');
out_vals=[A90,A30,I90,I30,E90,E30,A0, AIE0, AVV0];
