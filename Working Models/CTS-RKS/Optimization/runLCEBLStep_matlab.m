...
%     end
    ...
    % Calculate Error
    [rmse,QCFP]=LCCostFun4_S192803(jobParams)
...
dlmwrite(['designVector_' jobParams.task '_' num2str(jobParams.model) '.txt'],[DV reshape(rmse,1,[]) err],'-append')


%% thors debugging and email notification section
    dlmwrite('current_trial_metadata.txt',[fix(clock), reshape(rmse,1,[]) err],'-append')
