...
force_values=np.zeros((force_times,6))
counter=0
for HO in historyOutputName:
    for elem in elems:
        historyRegionName = historyRegionNameTemplate + '.' + str(elem)
...
# Export data
fname = filename+"_GSDynamics.csv"
f1=open(fname,'w')
f1.write("Time(s),   F(+)/E (deg),   Vr/Vl(+) (deg),   I/E(+) (deg),   M/L(+) (mm),   A(+)/P (mm),   S(+)/I (mm), F(+)/E (N*mm),   Vr/Vl(+) (N*mm),   I/E(+) (N*mm),   M/L(+) (N),   A(+)/P (N),   S(+)/I (N)")
f1.write("\n")
...
output_data={'TCS_to_FCS_TransMat':TCS_to_FCS_TransMat,'gskin':gskin,'Times':times,'GCS_to_FCS_TransMat':GCS_to_FCS_TransMat,'GCS_to_TCS_TransMat':GCS_to_TCS_TransMat,'Times_Force':force_times,'Loads':force_values}
fname = filename+"_GSDynamics.mat"
...
if want_plot=="1":
    ...
    # plt.plot(gskin[:,4],force_values[:,4])
